...
function [B1,B,B1_base, fval,test,test2,V_inv,S,V0,V1] = ident_het_fmins(ann_chosen,T,K,Uhat,x0,options,normalizer,pos_norm,ann_start)
...
[x, fval, exitflag, output] = fminsearch(fun,x0, options);  % quasi-newton method, exitflag 1: success, 0: solver stopped, 2: change in x smaller than step tolerance, 5:predicted decrease in obj fct was less than the function tolerance    
B1_base = x;
                      
% Gather test statistics
test  = S' /(V0+V1) * S;    % Test statistic for Wald Test used in model diagnostics
test2 = fval;              % Test whether there is a one-dimensional structural shock that characerises the difference between the two days
